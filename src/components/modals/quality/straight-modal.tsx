...
export const StraightModal = ({ match }: Props) => {
  const { isOpen, onClose, type } = useModal()

  const isModalOpen = isOpen && type === 'straight'

  const [imgSrc, setImgSrc] = useState('/images/quaility/straight.gif')

  useEffect(() => {
    if (isModalOpen) {
      setImgSrc(`/images/quaility/straight.gif?${Date.now()}`)
